...
async def upsert_evaluation(record: dict) -> None:
    ...
    async with get_cosmos_client() as cosmos:
        ...
        container = db.get_container_client(settings.AZURE_COSMOS_CONTAINER_EVALUATIONS)
...
async def upsert_feedback(record: dict) -> None:
    ...
    async with get_cosmos_client() as cosmos:
        db        = cosmos.get_database_client(settings.AZURE_COSMOS_DATABASE)
        container = db.get_container_client(settings.AZURE_COSMOS_CONTAINER_FEEDBACK)
        await container.upsert_item(record)
...
async def get_evaluation(answer_id: str, domain: str) -> dict | None:
    ...
    async with get_cosmos_client() as cosmos:
        ...
        container = db.get_container_client(settings.AZURE_COSMOS_CONTAINER_EVALUATIONS)
        try:
            item = await container.read_item(item=answer_id, partition_key=domain)
            return item
        except Exception:
            return None
...
async def get_domain_stats(domain: str) -> dict:
    ...
    async with get_cosmos_client() as cosmos:
        ...
        eval_stats, fb_stats = {}, {}
        async for item in db.get_container_client(
            settings.AZURE_COSMOS_CONTAINER_EVALUATIONS
        ).query_items(query=eval_q, parameters=params):
            eval_stats = item

        async for item in db.get_container_client(
            settings.AZURE_COSMOS_CONTAINER_FEEDBACK
        ).query_items(query=fb_q, parameters=params):
            fb_stats = item
