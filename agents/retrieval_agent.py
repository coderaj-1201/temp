...
@step
async def synthesize_answer(query: str, docs: list[SearchDocument]) -> tuple[str, float, list[SourceDocument]]:
    if not docs:
...
async def retrieval_workflow(request: OrchestratorRequest) -> RetrievalResult:
    ...
    answer, confidence, source_docs = await synthesize_answer(request.query, docs)
